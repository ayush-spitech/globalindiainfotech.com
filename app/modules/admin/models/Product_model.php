...
class Product_model extends MY_Model {
	...
	function add(){	
		...
		$editId=$this->input->post('product_id');
		...
		if($this->form_validation->run()==TRUE){									
			...
			$image='';
			$row=parent::getRecord('*',array("product_id"=>$editId));
			...
			if($image!=''){
				...
				$_POST['rowId']=$editId;			
				$lastId=parent::save($this->tbl_name,$aInput,'product_id'); 
				$response['msg']=$lastId;
			}else{
				$response['is_error']=1;
				$response['msg']='Product Image is required';
			}				
		}else{
			$response['is_error']=1;
