...
class Industry_model extends MY_Model {
	...
	function __construct(){
		parent::__construct();
		parent::setTable('industry');
	}

	function add(){	
		//debug($_FILES);
		$response=array('is_error'=>'0','class'=>'text-success','msg'=>'');
		$editId=$this->input->post('industry_id');
		...
		if($this->form_validation->run()==TRUE){									
			...
			$image='';
			$row=parent::getRecord('*',array("industry_id"=>$editId));
			...
			if($image!=''){
				...
				$_POST['rowId']=$editId;			
				$lastId=parent::save($this->tbl_name,$aInput,'industry_id'); 
				$response['msg']=$lastId;
			}else{
				$response['is_error']=1;
				$response['msg']='Industry Image is required';
			}				
		}else{
			$response['is_error']=1;
	...
	function get_list($str_select = '*',$aWhere=array()){
		...
		$this->db->from(tbl_prefix().'industry as t1');				
